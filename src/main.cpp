  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &planner](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		MyCar myCar(car_x, car_y, car_s, car_d, car_yaw, car_speed,
			    previous_path_x, previous_path_y);
		planner.resetMyCar(myCar);
		planner.planWaypoints(sensor_fusion);

		planner.generateSmoothTrajectory(next_x_vals,
						 next_y_vals);
